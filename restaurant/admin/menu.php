...
<?php 
    ...
    <div class="menucontainer"> 
      ...
      <div class="additem"> 
      <a href="Menucard/createItem.php" class="btnitem"><i class="fa fa-plus"></i> Add 
Item</a> 
      </div> 
       <form method="POST"> 
       <div class="row"> 
       <select name="search" id="search" class="form-control"> 
        ...
        <div class="col" style="text-align: right;height:20px;" > 
            <a href="menu-panel.php" class="btn btn-light">Menu Panel</a>
        </div> 
     </form> 
...
if (isset($_POST['search'])) { 
    if (!empty($_POST['search'])) { 
        $search = $_POST['search']; 
        $sql = "SELECT * FROM menu WHERE item_type LIKE '%$search%'  OR item_name 
LIKE '%$search%' OR item_id LIKE '%$search%' ORDER BY item_id;"; 
    } else { 
    ...
    if (mysqli_num_rows($result) > 0) { 
        ...
        while ($row = mysqli_fetch_array($result)) { 
            ...
            // Modify link with the pencil icon 
                $update_sql = "UPDATE menu SET item_name=?, item_type=?, item_category=?, 
item_price=?, item_description=? WHERE item_id=?"; 
 
            echo '<a class="icon" href="./Menucard/update.php?id='. $row['item_id'] .'" 
